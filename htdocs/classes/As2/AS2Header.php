...
class AS2Header implements Countable, ArrayAccess, Iterator {
    ...
    public function addHeaders($values) {
        foreach($values as $key => $value)
            $this->addHeader($key, str_replace($this->_header_replace, '', $value) );
    }
     ...
     *
     * New version modified by Enrico Valsecchi
     *
     * Leave '"' from in as2-from and as2-to
     * 
     * @param string  The content to parse
     * 
     * @return object  AS2Header instance
    ...
    public static function parseHttpRequest() {
        ...
        if( !function_exists('apache_request_headers') ) {
            ...
            foreach($_SERVER as $key => $value){
                if (strpos($key, 'HTTP_') === 0){
                    ...
                    $headers[$key] = str_replace(AS2Header::getHeaderReplace(), '', $value);
                }
            }
        ...
        {
            if(strtolower($key) == 'as2-from' || strtolower($key) == 'as2-to')
            {
                $value = str_replace(AS2Header::getHeaderReplace(), '', $headers[$key]);
                $headers[$key] = $value;
            }
        }
        return new self($headers);
    ...
    public static function getHeaderReplace()
    {
    	return array('"', '\'');
    }
