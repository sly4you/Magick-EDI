...
class AS2Server {
	...
	public static function handle($request = null)
	{
		...
		try {
			...
			if (is_null($request)){
				...
				/**
                 * **** Modified for check message sender/receiver combination.
                 */
				// Check if combination sender/receiver are good.
				if( !$partner_data = AS2Partner::checkPartnerCombination(trim($headers->getHeader('as2-from')), trim($headers->getHeader('as2-to'))) )
				{
					// log some informations
					AS2Log::error($headers->getHeader('message-id'), 'Ooops! The partner ' . $headers->getHeader('as2-from') . '#' . $headers->getHeader('as2-to') . " does not exists or are not active.");
					throw new AS2Exception('The partner ' . $headers->getHeader('as2-from') . '#' . $headers->getHeader('as2-to') . " does not exists or are not active.");
				}
				$as2from = $headers->getHeader('as2-from') . '#' . $headers->getHeader('as2-to');
				$as2to = $headers->getHeader('as2-to') . '#' . $headers->getHeader('as2-from');
				// Modify header from and to
				$headers->addHeader('as2-from', $as2from);
				$headers->addHeader('as2-to', $as2to);
				/**
                 * **** End of modified version.
                 */
				// main save action
				$filename = AS2Message::buildFileName();
				$message_id = AS2Message::saveInboxMessage( false, $headers->getHeader('as2-from'), $headers->getHeader('as2-to'), $headers->getHeader('message-id'), $headers->getHeader('subject'), $headers, $data, $filename );
				...
				if ($decrypted) {
					...
					AS2Message::saveInboxMessage($message_id, $headers->getHeader('as2-from'), $headers->getHeader('as2-to'), '', '', array(), $content, $filename.'.decrypted', 'decrypted');
				}
		...
		if ($object instanceof AS2Message || (!is_null($error) && !($object instanceof AS2MDN))){
			...
			AS2Log::info($headers->getHeader('message-id'), 'Incoming transmission is a Message.');
			...
			try {
				if (is_null($error)){
					...
					foreach($files as $key => $file){
						...
						AS2Message::saveInboxMessage($message_id, $headers->getHeader('as2-from'), $headers->getHeader('as2-to'), '','', array(), $content, $filename . '.payload_'.$key, 'payload');
						AS2Log::info($headers->getHeader('message-id'), 'Payload #' . ($key+1) . ' : ' . round(strlen($content) / 1024, 2) . ' KB / "' . $file['filename'] . '".');
						$payload_filename[] = $filename . '.payload_'.$key;
					}
					...
					AS2Message::updateMessageOnDb( $message_id, -1, $error );
					throw $error;
				}
			}
			catch(Exception $e){
		...
		elseif ($object instanceof AS2MDN) {
			...
			ob_start();
			print_r( $headers->getHeaders()->toFormattedArray() );
			$out1 = ob_get_contents();
			ob_end_clean();
			
			AS2Log::info($headers->getHeader('message-id'), 'Incoming transmission is a MDN.');
		}
		else {
			AS2Log::error($headers->getHeader('message-id'), 'Malformed data.');
		}
		...
		if (!is_null($mdn)){
			if (!$headers->getHeader('receipt-delivery-option')) {
				...
				AS2Log::info($headers->getHeader('message-id'), 'An AS2 MDN has been sent.');
				AS2Message::updateMessageOnDb( $message_id, 3, __MESSAGE_MDN_SENDED__ );
			}
			else {
				// ASYNC method
				...
				if ($result['info']['http_code'] == '200'){
					...
					AS2Log::error($headers->getHeader('message-id'), 'An error occurs while sending MDN message : ' . $result['info']['http_code']);
					AS2Message::updateMessageOnDb( $message_id, -1, __MESSAGE_MDN_ERROR__ );
				}
			}
		}
		else
		{
			AS2Message::updateMessageOnDb( $message_id, 3, __MESSAGE_MDN_NOT_REQUIRED__ );
		}
		// O.K., all steps of AS2 Server are good.
		// If customer have choiced to receive automatic e-mail when system receive a message, send!
		if( $partner_data['partner_auto_email'] == 1 && $payload_filename )
		{
			$local_customer_data = AS2Partner::getPartnerCombination($as2to);
			AS2Message::sendReceivedMessageByEmail($local_customer_data['user_id'], $local_customer_data['partner_org_name'], $local_customer_data['partner_email'], $partner_data['partner_org_name'], $payload_filename);
		}
		return $request;
	}
	...
	*/
	...
	/**
     * Close current HTTP connection and wait some seconds 
     * 
