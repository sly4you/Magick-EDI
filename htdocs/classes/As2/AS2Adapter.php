...
class AS2Adapter {
    ...
    public function __construct($partner_from, $partner_to){
        try {
            $this->partner_from = AS2Partner::getPartner($partner_from . '#' . $partner_to);
        }
        catch(Exception $e){
            throw new AS2Exception('Sender AS2 id "' . $partner_from . '#' . $partner_to . '" is unknown.');
        }

        try {
            $this->partner_to   = AS2Partner::getPartner($partner_to . '#' . $partner_from);
        }
        catch(Exception $e){
            throw new AS2Exception('Receiver AS2 id "' . $partner_to . '#' . $partner_from . '" is unknown.');
        }
    }
