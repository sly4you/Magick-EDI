 ...
 * Usage example:
 * <code>
 * require_once 'EDI_EANCOM_MappingProvider.php';
 * 
 * $spec = EDI_EANCOM_MappingProvider::find('UNB');
 ...
 */
class EDI_EANCOM_MappingProvider
{
	// Constants {{{
     ...
     */
	const LATEST_DIRECTORY = 'EURITMO';
	...
	const LEVEL_TRANSLATE              = 'translate';

	/**
     * Type constants.
     */
	const TYPE_SERVICE = 'EDI_EANCOM_Service_';
	const TYPE_USER    = 'EDI_EANCOM_';
     ...
     */
	public static function find($id, $dir=null, $syntax=null,$message=null)
	{
		$node = false;
		if (preg_match('/^translate_.*/', $id, $tokens)) {
			// Special case for translation rules
			// form is translate_EDIFACT_D96A_INVOIC_BGM
			$node = self::getSpecNode($id, self::LEVEL_TRANSLATE, self::TYPE_USER, $dir, $syntax);
		} else if (preg_match('/[A-Z]{6}/', $id)) {
			$id = strtolower($id);
			// we have a messages
			if ($id == 'autack' || $id == 'contrl' || $id == 'keyman') {
				// service message
				$node = self::getSpecNode($id, self::LEVEL_MESSAGE, self::TYPE_SERVICE, $dir, $syntax);
			} else {
				// user message
				$node = self::getSpecNode($id, self::LEVEL_MESSAGE, self::TYPE_USER, $dir, $syntax);
			}
		} else if (preg_match('/^([A-Z])[A-Z]{2}$/', $id, $tokens)) {
			// we have a segment
			$id = strtoupper($id);
			// it's a user segment
			$node = self::getSpecNode($id, self::LEVEL_SEGMENT, self::TYPE_USER, $dir, $syntax, $message);
			...
			if (is_numeric($tokens[1])) {
				...
				if ((int)$id < 1000) {
					...
					$node = self::getSpecNode($id, self::LEVEL_DATA_ELEMENT, self::TYPE_SERVICE, $dir, $syntax);
				} else {
					// it's a user data element
					$node = self::getSpecNode($id, self::LEVEL_DATA_ELEMENT, self::TYPE_USER, $dir, $syntax);
				}
			} else if ($tokens[1] == 'S') {
				// service composite data element
				$node = self::getSpecNode($id, self::LEVEL_COMPOSITE_DATA_ELEMENT, self::TYPE_SERVICE, $dir, $syntax);
			} else {
				// user composite data element
				$node = self::getSpecNode($id, self::LEVEL_COMPOSITE_DATA_ELEMENT, self::TYPE_USER, $dir, $syntax);
			}
		}
		if ($node) {
			return $node;
		} else {
			throw new EDI_Exception( 'no mapping found for identifier "' . $id . '"', EDI_Exception::E_EDI_MAPPING_ERROR );
		}
	}
     ...
     * @throws EDI_Exception
     */
	public static function getSpecNode($id, $level, $type=null, $dir=null, $syntax=null,$message=null)
	{
		if (is_dir('@data_dir@')) {
			$basedir = '@data_dir@/';
		} else {
		...
		if ($level == self::LEVEL_MESSAGE) {
			...
			if ($type == self::TYPE_SERVICE) {
				...
				$f   = $basedir . self::TYPE_USER . $dir . "/service_codes_v{$syn}.xml";
			} else {
				$f = $basedir . self::TYPE_USER . $dir . "/codes.xml";
			}
		} else if ($level == self::LEVEL_TRANSLATE) {
			// form is translate_EDIFACT_D96A_INVOIC_BGM
			$p = preg_match('/^translate_(\w+)_(\w+)_(\w{6})_(\w{3})/', $id, $tokens);
			if ($p == FALSE)
			die ("FERMO $id");
			$f .= "/".$tokens [1]."_".$tokens [2]."/".strtolower ($tokens [3]).".xml";
			$id = $tokens [4];
		} else {
			$f .= ".xml";
		}
		...
		if (!file_exists($f)) {
			throw new EDI_Exception('mapping file "' . $f . '" not found', EDI_Exception::E_EDI_MAPPING_ERROR);
		}
		if (!($xml = simplexml_load_file($f))) {
			throw new EDI_Exception('invalid xml mapping file "' . $f . '"', EDI_Exception::E_EDI_MAPPING_ERROR);
		}
		...
		foreach ($root as $elt) {
			if ((string)$elt['id'] == $id) {
				// In EANCOM we can have the same segment name with different fields (sigh!)
				// See NAD in INVOIC and DESADV
				// So in the segments.xml we added the "message" attribute. In case it's there, check over the message
				// we're currently parsing
				if ((string)$elt['message'] != "" && strtolower($message) != strtolower((string)$elt['message']))
				continue;
				self::$xmlspecs[$cacheId] = $elt;
				return $elt;
			}
		}
		throw new EDI_Exception( 'mapping identifier "' . $id . '" not found', EDI_Exception::E_EDI_MAPPING_ERROR );
	}
