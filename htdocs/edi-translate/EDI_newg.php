 ...
 * @filesource
 */
// Path definition for all class document
define('__AB_PATH__', dirname(__FILE__) . '/' );
define('__DIR_EDI_STANDARD_DATA__', __AB_PATH__ . 'data/' );
...
final class EDI
{
			...
			if(array_key_exists('EDI_EANCOM_EURITMO', $this->available_edi_standard))
			{
				...
				foreach($this->available_edi_standard['EDI_EANCOM_EURITMO'] as $key => $single_message_type)
				{
					...
					if( preg_match($match, $edi_string) )
					{
						...
						$this->interch = $this->parser->parse($this->edi_file, strtoupper($single_message_type));
			...
			foreach ($this->available_edi_standard as $key => $single_standard_available_messages)
			{
				...
				foreach($single_standard_available_messages as $k => $single_message_type)
				{
					...
					if( preg_match($match_standard, $edi_string) )
					{
						...
						$this->interch = $this->parser->parse($this->edi_file, $message_type);
	...
	{
		$include_file = __AB_PATH__ . 'EDI/' . $standard . '/Parser.php';
	...
	{
		include_once __AB_PATH__ . 'EDI/' . $standard . '/Elements.php';
